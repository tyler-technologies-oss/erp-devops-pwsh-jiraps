...
Describe "Remove-JiraIssueWatcher" -Tag 'Unit' {
    ...
    InModuleScope Tyler.DevOps.JiraPS {
        ...
        Mock Invoke-JiraMethod -ModuleName Tyler.DevOps.JiraPS -ParameterFilter {$Method -eq 'DELETE' -and $URI -eq "$jiraServer/rest/api/2/issue/$issueID/watchers?username=fred"} {
